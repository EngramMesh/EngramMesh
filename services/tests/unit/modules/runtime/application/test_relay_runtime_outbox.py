...
from engrammesh.modules.runtime.application.contracts import RelayRuntimeOutboxCommand
...
def make_runtime_event(event_id: EventId) -> EventEnvelope:
...
class RecordingRuntimeRelayStore:
    def __init__(
        self,
        calls: list[str],
        *,
...
class FailingAfterFirstPublisher:
...
async def test_empty_batch_returns_zeros_and_remaining_count() -> None:
    calls: list[str] = []
    store = RecordingRuntimeRelayStore(calls, remaining=3)
    handler = RelayRuntimeOutboxEventsHandler(
        clock=MustNotBeUsed(),
        store=store,
        publisher=MustNotBeUsed(),
    )

    result = await handler.handle(RelayRuntimeOutboxCommand(batch_size=10))
...
async def test_full_batch_dispatches_marks_and_returns_counts() -> None:
    calls: list[str] = []
    event_1 = make_runtime_event(EVENT_ID_1)
    event_2 = make_runtime_event(EVENT_ID_2)
    store = RecordingRuntimeRelayStore(
        calls,
...
async def test_publish_failure_reraises_without_marking() -> None:
    ...
    with pytest.raises(AdapterFailure) as raised:
        await handler.handle(RelayRuntimeOutboxCommand(batch_size=10))
...
@pytest.mark.asyncio
async def test_relay_runtime_outbox_mid_batch_failure_does_not_mark() -> None:
    calls: list[str] = []
    events = (make_runtime_event(EVENT_ID_1), make_runtime_event(EVENT_ID_2))
    store = RecordingRuntimeRelayStore(calls, events=events, remaining=2)
    publisher = FailingAfterFirstPublisher(calls)
    handler = RelayRuntimeOutboxEventsHandler(
        clock=FixedClock(calls),
        store=store,
        publisher=publisher,
