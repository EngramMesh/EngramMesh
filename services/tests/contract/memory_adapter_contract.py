...
async def assert_twenty_concurrent_duplicates_converge(
    ...
    async def append(identifier: int) -> tuple[MemoryId, bool]:
        episode = make_episode(identifier, idempotency_key="shared-key")
        async with harness.unit_of_work_factory.create() as unit_of_work:
            result = await unit_of_work.episodes.append(episode)
            if result.created:
                await unit_of_work.outbox.publish(
                    make_event(identifier, episode=episode)
                )
            await unit_of_work.commit()
        return result.episode_id, result.created

    results = await asyncio.gather(*(append(value) for value in range(1, 21)))
...
async def assert_cancellation_while_queued_releases_lock(
    ...
    async def wait_for_transaction() -> None:
        ...
        waiter_started.set()
        async with harness.unit_of_work_factory.create():
            waiter_entered = True
...
MEMORY_ADAPTER_CONTRACTS: tuple[
