"""Async PostgreSQL connection pool for the runtime adapter."""
...
class PostgresRuntimeConnection:
    """Own a psycopg async pool and apply runtime schema migrations on first use."""
    ...
    async def connection(self) -> AsyncIterator[AsyncConnection]:
        ...
        async with self._migration_lock:
            pool = self._pool
            if pool is None:
                msg = "postgres runtime database is not open"
    ...
    async def _ensure_migrations(self) -> None:
        async with self._migration_lock:
            if self._migrations_applied:
                return
            await asyncio.to_thread(_apply_runtime_migrations_from_dsn, self._dsn)
            self._migrations_applied = True


def _apply_runtime_migrations_from_dsn(dsn: str) -> None:
